...
class SavedWorkload:
    ...
    def save_workload(user_id: int, title: str, data: Dict[str, Any], comment: str = None) -> int:
        ...
        try:
            ...
            # Сохраняем ВСЕ строки расчета, включая те, где Учитывать = False
            batch_size = 100
            ...
            for i in range(0, total_rows, batch_size):
                ...
                for row in batch:
                    ...
                    row_params = (
                        ...
                        row.get('Комментарии', ''),
                        1 if row.get('Учитывать', True) else 0  # По умолчанию True, но сохраняем реальное значение
                    )
    ...
    @staticmethod
    def get_workload_by_id(workload_id: int, include_hidden: bool = True) -> Optional[Dict[str, Any]]:
        """
        Получение расчета нагрузки по ID
        
        Args:
            workload_id: ID расчета
            include_hidden: Включить строки где Учитывать = False
        ...
        WHERE 
            РасчетID = ?
        ORDER BY ИндексДисциплины, Семестр, ВидРаботы
        """
        
        # ВСЕГДА загружаем все строки, но можем потом отфильтровать
        ...
        for row in rows_results:
            ...
            # Добавляем строку в список (даже если Учитывать = False)
    ...
    def update_workload(workload_id: int, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Вставляем ВСЕ обновленные строки расчета
            batch_size = 100
            total_rows = len(calculated_data)
            ...
            for i in range(0, total_rows, batch_size):
                ...
                for row in batch:
                    ...
                    row_params = (
                        ...
                        row.get('Пункт приказа', ''),
                        row.get('Комментарии', ''),
                        1 if row.get('Учитывать', True) else 0  # ВАЖНО: сохраняем реальное значение
                    )
